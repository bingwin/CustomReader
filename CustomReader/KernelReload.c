...
BOOL GetWindowsRootName(WCHAR *WindowsRootName)
{
	...
	ObjectNameBuffer = (WCHAR*)ExAllocatePoolWithTag(NonPagedPool,260*2,'ILVV');
	...
	ULONG CreateOptions)
{
	...
	FileNodeName = (WCHAR*)ExAllocatePoolWithTag(NonPagedPool,260*2,'ILVV');
...
BOOL ImageFile(BYTE *FileBuffer,BYTE **ImageModuleBase)
{
	...
	ImageBase=ExAllocatePoolWithTag(NonPagedPool,SizeOfImage,'ILVV');
	if (ImageBase==NULL)
	{
		return FALSE;
	}
...
PVOID GetKernelModuleBase(PDRIVER_OBJECT DriverObject,CHAR *KernelModuleName)
{
	...
	Buffer=ExAllocatePoolWithTag(NonPagedPool,260*2,'ILVV');
	if (Buffer==NULL)
	{
		return NULL;
	}
...
BOOL PeReload(WCHAR* wszFullPath,DWORD ulKernelBase,BYTE** ulReloadImageBase,PDRIVER_OBJECT DeviceObject)
{
	...
	FileBuffer = (BYTE*)ExAllocatePoolWithTag(PagedPool,Length,'ILVV');
	if (FileBuffer == NULL)
	{
		ZwClose(hFile);
		return FALSE;
