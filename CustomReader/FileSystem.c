...
// 			  IN ULONG EaLength)
{
	...
	WORD FileObjectSize = (WORD)gStructOffset.FileObjectSize;
	...
	FileNameBuffer = (WCHAR *)ExAllocatePoolWithTag(NonPagedPool,FilePath->MaximumLength,'ILVV');
	...
	if (gWinVersion ==WINDOWS_VERSION_VISTA_2008||gWinVersion==WINDOWS_VERSION_7_7000||gWinVersion==WINDOWS_VERSION_7_7600_UP)
	{
		PLIST_ENTRY IrpList;
...
NTSTATUS
IrpClose(IN PFILE_OBJECT FileObject)
{
	NTSTATUS ntStatus;
	IO_STATUS_BLOCK IoStatusBlock;
	PIRP Irp;
